...
public class ImpCustomerDAO implements DAOCustomer {
    ...
    public Customer getCustomerByName(String username) {
        ...
        while (cursor.hasNext()) {
            ...
            cus.setRegistered_date(CalculatorService.getDateTime(obj.get("created_at") + ""));
        }
        return cus;
    }
    ...
    public List<DataCollection> getListRoomCanceled(String username) {
        ...
        new ImpActivityDAO().getAllActivityByUserName(username).stream().filter((act) -> (act.getName().equals("Cancel Room"))).forEach((act) -> {
    ...
    public double getAvgStarRoomFeedback(String username) {
        ...
        return CalculatorService.round(star * 1.0 / count, 2);
    }

    public int getTotalStarRoomFeedback(String username) {
        int star = 0;
        star = new ImpActivityDAO().getAllActivityByUserName(username).stream().filter((act) -> (act.getName().equals("Feedback Room"))).map((act) -> act.getNote().charAt(21) - 48).reduce(star, Integer::sum);
        return star;
    }
    ...
    public double getTotalStarFeedback(String username) {
        int star = 0;
        star = new ImpActivityDAO().getAllActivityByUserName(username).stream().filter((act) -> (act.getName().equals("Feedback"))).map((act) -> act.getNote().charAt(12) - 48).reduce(star, Integer::sum);
        return star;
    }
    ...
    public List<FeedbackRoom> getListFeedbackRoom(String username) {
        ...
        new ImpActivityDAO().getAllActivityByUserName(username).stream().filter((act) -> (act.getName().equals("Feedback Room"))).forEach((act) -> {
